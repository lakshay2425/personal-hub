...
interface SubQuestionHeaderProps {
  questionText: string;
...
export function SubQuestionHeader({
  questionText,
  textClassName,
  hasChildren,
  isChildrenCollapsed,
    ...
    <div className="min-w-0 flex-1">
      <div className="flex items-start gap-1.5">
        <button
          ...
          aria-label={
            isChildrenCollapsed ? "Show sub-questions" : "Hide sub-questions"
          }
      ...
      <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 pl-6">
        ...
        >
          {isChildrenCollapsed
            ? `Show ${descendantCount} sub-question${descendantCount === 1 ? "" : "s"}`
            : `Hide ${descendantCount} sub-question${descendantCount === 1 ? "" : "s"}`}
        </button>
        {meta}
      </div>
    </div>
